...
public class RenderGhastQueen extends RenderLiving
{
    ...
    protected ResourceLocation func_110867_a(final EntityGhastQueen par1EntityGhast) {
        return par1EntityGhast.func_110182_bF() ? RenderGhastQueen.ghastqueenShootingTextures : RenderGhastQueen.ghastqueenTextures;
    }
    
    protected void preRenderGhast(final EntityGhastQueen par1EntityGhast, final float par2) {
        float f1 = (par1EntityGhast.prevAttackCounter + (par1EntityGhast.attackCounter - par1EntityGhast.prevAttackCounter) * par2) / 20.0f;
        if (f1 < 0.0f) {
            f1 = 0.0f;
        }
